...
import static com.ruoyi.common.redis.configure.FastJson2JsonRedisSerializer.DEFAULT_CHARSET;
...
public class RedisService {
    ...
    public <T> T getCacheObject(final String key, Class<T> clazz) {
        try {
            // 1. 获取原始字节数据
            byte[] valueBytes = (byte[]) redisTemplate.execute((RedisCallback<byte[]>) conn ->
                    conn.get(key.getBytes(DEFAULT_CHARSET))
            );
            if (valueBytes == null) {
                return null;
            }
            ...
            log.error("反序列化失败，键: {}, 错误信息: {}", key, e.getMessage());
            return null;
        }
    }
